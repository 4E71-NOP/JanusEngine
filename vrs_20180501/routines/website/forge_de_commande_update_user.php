...
if ( is_uploaded_file($_FILES[$pv['au']]['tmp_name']) ) {
	if ( $_FILES[$pv['au']]['error'] ) {
		...
		if (!(in_array( $pv['au_extension'] , $pv['au_extension_autorisees'] ) ) ) { 
			...
			$pv['au_rep_dest'] = dirname( "../websites-datas/".$site_web['sw_repertoire']."/data/images/avatars/uploaded/." ). "/";
			...
			$handle = opendir("../websites-datas/".$site_web['sw_repertoire']."/data/images/avatars/uploaded/");
			...
			foreach ( $repertoire_liste_fichier as $A ) {
				$pv['au_count']++;
				if ( $pv['au_count'] > 3 ) { unlink ("../websites-datas/".$site_web['sw_repertoire']."/data/images/avatars/uploaded/".$A); }
			}
			unset (
			$handle,
