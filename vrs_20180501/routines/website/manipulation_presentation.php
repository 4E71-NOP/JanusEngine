...
//	Creation
		case 1: 
			...
			if ( $R['ERR'] != 1 ) {
				$R['id'] = manipulation_trouve_id_suivant ( $SQL_tab_abrege['presentation'] , "pres_id" );
				...
				$requete1 = "INSERT INTO ".$SQL_tab_abrege['presentation']." VALUES ".$requete_valeurs.";";
...
//	modification
		case 1: 
			...
			if ( $R['ERR'] != 1 ) {
				...
				$requete1 = "UPDATE ".$SQL_tab_abrege['presentation']." SET ".$requete_colonne." WHERE pres_id = '".$R['id']."';";
				manipulation_traitement_requete ( $requete1 );
...
//	Suppression
		case 3:
			...
			if ( $R['ERR'] != 1 ) {
				...
				$requete1= "
				DELETE FROM ".$SQL_tab_abrege['presentation']." 
				WHERE pres_id ='".$R['id']."'
				);";
				manipulation_traitement_requete ( $requete1 );

				$requete1= "
				DELETE FROM ".$SQL_tab_abrege['theme_presentation']." 
				WHERE pres_id ='".$R['id']."'
				);";
				manipulation_traitement_requete ( $requete1 );
...
//	Assignation a un theme
		case 4:
			...
			if ( $R['ERR'] != 1 ) {
				...
				$R['theme_pres_id'] = manipulation_trouve_id_suivant ( $SQL_tab_abrege['theme_presentation'] , "theme_pres_id" );
				if ( $R['defaut'] == 1 ) {
					requete_sql($_REQUEST['sql_initiateur'],"
					UPDATE ".$SQL_tab_abrege['theme_presentation']." SET 
					pres_defaut = '0' 
					WHERE theme_id = '".$R['assignee_a']."'
					;");
				}
				$requete1= "
				INSERT INTO ".$SQL_tab_abrege['theme_presentation']." VALUES (
				'".$R['theme_pres_id']."',
				'".$R['assignee_a']."',
				'".$R['id']."',
				'".$R['defaut']."'
...
//				$pv['fieldlist'] = array ( "ligne", "minimum_x", "minimum_y", "module_nom", "type_calcul",  "position_x", "position_y", "dimenssion_x", "dimenssion_y" );
				...
				$R['pres_cont_id'] = manipulation_trouve_id_suivant ( $SQL_tab_abrege['presentation_contenu'] , "pres_cont_id" );
				...
				$requete_valeurs = substr ( $requete_valeurs , 0 , -1 ) . ") ";
				$requete1 = "INSERT INTO ".$SQL_tab_abrege['presentation_contenu']." VALUES ".$requete_valeurs.";";
