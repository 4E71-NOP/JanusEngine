...
function manipulation_site () {
	global $site_web, $SQL_tab, $SQL_tab_abrege, $db, $db_, $langues;
	$R = &$_REQUEST['M_SITWEB'];
...
// --------------------------------------------------------------------------------------------
	case 1:
		...
		if ( $R['ERR'] != 1 ) {
			...
			$R['sw_id'] = manipulation_trouve_id_suivant ( $SQL_tab_abrege['site_web'] , "sw_id" );
			...
			$requete1 = "INSERT INTO ".$SQL_tab_abrege['site_web']." VALUES ".$requete_valeurs.";";
	...
	case 2:
		if ( $_REQUEST['site_context']['site_id'] == 0 ) {
			...
			if ( $R['ERR'] != 1 ) {
				...
				$requete1 = "UPDATE ".$SQL_tab_abrege['site_web']." SET ".$requete_colonne." WHERE sw_id = '".$_REQUEST['site_context']['site_id']."';";
				manipulation_traitement_requete ( $requete1 );
				...
				if ( strlen($R['ajout_lang']) != 0 ) {
					...
					$dbquery = requete_sql($_REQUEST['sql_initiateur'],"SELECT * FROM ".$SQL_tab_abrege['site_langue']." WHERE site_id = '".$R['id']."';");
					...
					if ( $R['ERR'] != 1 ) {
						$R['site_lang_id'] = manipulation_trouve_id_suivant ( $SQL_tab_abrege['site_langue'] , "site_lang_id" );
						requete_sql($_REQUEST['sql_initiateur'],"
						INSERT INTO ".$SQL_tab_abrege['site_langue']." VALUES ( 
						'".$R['site_lang_id']."',
				...
				if ( strlen($R['suppression_lang']) != 0 ) {
					...
					$requete1 = "
					DELETE FROM ".$SQL_tab_abrege['site_langue']."
...
//	Suppression de site
	case 3:
		$dbquery = requete_sql($_REQUEST['sql_initiateur'],"
		SELECT * FROM ".$SQL_tab_abrege['site_web']." 
		WHERE sw_nom = '".$R['nom']."'
		;");
		if ( num_row_sql($dbquery) == 0 ) {
		...
		if ( $R['ERR'] != 1 ) {
			$requete1 = "UPDATE ".$SQL_tab_abrege['site_web']." 
...
//	show
	case 5:
		...
		if ( $R['nom'] == "Nouveau site" ) {
			$dbquery = requete_sql($_REQUEST['sql_initiateur'],"
			SELECT sw_nom FROM ".$SQL_tab_abrege['site_web']." 
			;");
