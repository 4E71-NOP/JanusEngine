...
/*Hydre-licence-fin*/
$ligne++;
$tampon_commande_buffer[$ligne] = "update theme name \"".$_REQUEST['M_THEME']['nom']. "\"	\n";
...
for ( $pv['i'] = 1 ; $pv['i'] <= 30 ; $pv['i']++ ) {
	...
	if ( strlen ($_REQUEST['M_THEME']['theme_bloc_'.$pv['j'].'_nom']) > 0 ) {
		$tampon_commande_buffer[$ligne] .= "block_".$pv['j']."_name		\"".$_REQUEST['M_THEME']['theme_bloc_'.$pv['j'].'_nom']."\"	\n";
	}
	if ( strlen ($_REQUEST['M_THEME']['theme_bloc_'.$pv['j'].'_texte']) > 0 ) {
		$tampon_commande_buffer[$ligne] .= "block_".$pv['j']."_text		\"".$_REQUEST['M_THEME']['theme_bloc_'.$pv['j'].'_texte']."\"	\n";
	}
}
for ( $pv['i'] = 0 ; $pv['i'] <= 9 ; $pv['i']++ ) {
	$pv['j'] = sprintf("%02u", $pv['i'] );
	if ( strlen ($_REQUEST['M_THEME']['theme_bloc_'.$pv['j'].'_menu']) > 0 ) {
		$tampon_commande_buffer[$ligne] .= "block_".$pv['j']."_menu		\"".$_REQUEST['M_THEME']['theme_bloc_'.$pv['j'].'_menu']."\"	\n";
	}
}
