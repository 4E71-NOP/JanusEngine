...
// Trouve la position du premier menu qui servira de base.
	for ( var Rcn in Tab ) {
		switch ( Tab[Rcn].typ ) {
		case 'div':	
			if ( Tab[Rcn].p == 'racine' ) { var PositionRacine = LocaliseElement ( Tab[Rcn].id ); }
		}
	}

	for ( var ptr in Tab ) {
		var Obj = Gebi ( String(Tab[ptr].id ) );

		Obj.MWM_Bakup_style_marginTop		= Obj.style.marginTop;
		Obj.MWM_Bakup_style_opacticy		= Obj.style.opacity;
		Obj.MWM_Bakup_style_MozOpacity		= Obj.style.MozOpacity;
		Obj.MWM_Bakup_style_filter			= Obj.style.filter;
		Obj.MWM_Bakup_style_zIndex			= Obj.style.zIndex;
		Obj.MWM_Bakup_style_letterSpacing	= Obj.style.letterSpacing;
		Obj.MWM_Bakup_style_lineHeight		= Obj.style.lineHeight;
		Obj.MWM_Bakup_style_lineHeightN		= Number( Obj.style.lineHeight.replace( RegExp ('px', 'g') , '' ));
		Obj.MWM_Bakup_style_width			= Number ( Tab[ptr].width );
		Obj.MWM_Bakup_style_height			= Obj.clientHeight;

		Obj.MWM_Racine = 1;
		Obj.MWM_Type = Tab[ptr].typ;
		Obj.MWM_Dossier = Number ( Tab[ptr].dos );
		Obj.MWM_EtatMenu = 1;
		Obj.MWM_AnimPrevue = 0;
		Obj.MWM_Progression = 0;
		Obj.MWM_Sens = 0;
		Obj.MWM_Niveau = Number ( Tab[ptr].niv );
		Obj.MWM_Deco = Number ( Tab[ptr].deco );
		Obj.MWM_Anim = Tab[ptr].anim;
		Obj.MWM_Parent = Gebi ( Tab[ptr].p );
		Obj.MWM_StopPropagation = 1;
...
// Ajoute des sentinelles d'évennement au lieu de remplacer (Obj.onmouseover='xxx') pour laisser les animations déjà en place.
		...
		switch ( Tab[ptr].typ ) {
		case 'div':
			if ( Tab[ptr].p != 'racine' && Tab[ptr].niv != 0) {
				...
				var CotesDuParent = LocaliseElement ( Tab[ptr].p );
				...
				case 10 :	
					CotesDuParent = LocaliseElement ( Tab[ptr].AlignRef );
				...
				Obj.MWM_Bakup_style_top				= Obj.style.top;
				Obj.MWM_Bakup_style_left			= Obj.style.left;
			...
			CalculeDecoModule ( TabInfoModule , Tab[ptr].id );
			}
		break;
		case 'a':
			if ( Obj.MWM_Dossier == 1 ) { 
				Obj.MWM_Fils = Tab[ptr].f.a1;
				var ObjF = Gebi ( Obj.MWM_Fils );
				var ObjP = Obj.parentNode.style;
				
				JSJournal[dbgPopMenu]('ObjP.left=' + ObjP.left + 
...
function CustomEventMouseOver ( e ) {
	...
	case 1:
		...
		case 'MSIE7':	
			e.cancelBubble = true;	
			e.returnValue = false;	
			if ( e.srcElement.MWM_StopPropagation ) { var CibleFinale = e.srcElement; }
			else { var CibleFinale = e.srcElement.parentNode; }	
		break;
		default:		
			e.stopPropagation();	
			e.preventDefault();
			if ( e.target.MWM_StopPropagation ) { var CibleFinale = e.target; }
...
function GestionMenu ( Obj , TypeEve ) {
	...
	case 'a':		
		...
		if ( Obj.MWM_Dossier == 1 ) {
			a = a + 8; b += '8_';
			var ObjF = Gebi ( Obj.MWM_Fils );
			if ( ObjF.MWM_EtatMenu != 0 )	{ a = a + 4; b += '4_'; }
			if ( ObjF.MWM_AnimPrevue == 1 )	{ a = a + 2; b += '2_'; }
			if ( ObjF.MWM_Progression > 0 )	{ a = a + 1; b += '1_'; }
		}
	break;
	case 'div':
		if ( Obj.MWM_EtatMenu != 0 )	{ a = a + 4; b += '4_'; }
		if ( Obj.MWM_AnimPrevue == 1 )	{ a = a + 2; b += '2_'; }
		if ( Obj.MWM_Progression > 0 )	{ a = a + 1; b += '1_'; }
	break;
	}
	...
	if ( Np == 1 ) {
		if ( Obj.MWM_Type == 'a' ) {
			ObjF = Gebi ( Obj.MWM_Fils );
			switch ( AFils ) {
			case 10 :
				ObjF.MWM_Sens = 0;	ObjF.MWM_AnimPrevue = 1;	Ptr = ObjF.MWM_Anim;
				ObjF.MWM_AnimationEnCoursOUT = setTimeout( 'CacheSousMenuExplorateur ( \'' + ObjF.id + '\' );' , ChoixAnimation[Ptr].sca );
			break;
			case 11 :
				ObjF.MWM_Sens = 1;	ObjF.MWM_AnimPrevue = 1;	Ptr = ObjF.MWM_Anim;
		...
		while ( Np == 1 ) {
			if ( Obj.MWM_Racine == 1 || Obj.MWM_Niveau == 0 ) { Np = 0; }
			if ( Obj.MWM_Type == 'div' && Np == 1 ) {
				switch ( AParents ) {
				case 10 :
					NettoyageTimers ( Obj , 0 );
					Obj.MWM_Sens = 0;	Obj.MWM_AnimPrevue = 1;	Ptr = Obj.MWM_Anim;
					Obj.MWM_AnimationEnCoursOUT = setTimeout( 'CacheSousMenuExplorateur ( \'' + Obj.id + '\' );' , ChoixAnimation[Ptr].sca );
				break;
				case 11 :
					NettoyageTimers ( Obj , 1 );
					Obj.MWM_Sens = 1;	Obj.MWM_AnimPrevue = 1;	Ptr = Obj.MWM_Anim;
...
function AfficheSousMenuExplorateur ( ObjId ) {
	var Obj = Gebi( ObjId ), Ptr = Obj.MWM_Anim;
...
function NettoyageTimers ( Obj , Sens ) {
	var a = ( Sens * 2 );
	if ( Obj.MWM_Progression > 0 ) { a = a + 1; }
	switch ( a ) {
	case 0 :	clearTimeout( Obj.MWM_AnimationEnCoursIN );		Obj.MWM_Sens = 0;	Obj.MWM_AnimPrevue = 0;	break;
	case 1 :													Obj.MWM_Sens = 0;							break;
	case 2 :	clearTimeout( Obj.MWM_AnimationEnCoursOUT );	Obj.MWM_Sens = 1;	Obj.MWM_AnimPrevue = 0;	break;
	case 3 :													Obj.MWM_Sens = 1;							break;
	}
}
...
function GestionnaireAnimation ( ObjId ) {
	...
	if ( Obj.MWM_Progression < 1 ) {
		...
		switch ( Obj.MWM_Sens ) {
		case 0 :	clearTimeout( Obj.MWM_AnimationEnCoursIN );		Obj.MWM_AnimationEnCoursOUT = setTimeout( 'GestionnaireAnimation ( \'' + Obj.id + '\' );', ( ChoixAnimation[Ptr].scd / NombreDePas ) );	Obj.style.zIndex = Obj.MWM_Bakup_style_zIndex - 1; break;
		case 1 :	clearTimeout( Obj.MWM_AnimationEnCoursOUT );	Obj.MWM_AnimationEnCoursIN = setTimeout( 'GestionnaireAnimation ( \'' + Obj.id + '\' );', ( ChoixAnimation[Ptr].ecd / NombreDePas ) );	Obj.style.zIndex = Obj.MWM_Bakup_style_zIndex + 1; break;
		}
		Obj.MWM_Progression = Obj.MWM_Progression + ( 1 / NombreDePas );
	}
	else {
		switch ( Obj.MWM_Sens ) {
		case 0 :
		...
		Obj.style.top 			= Obj.MWM_Bakup_style_top;
		Obj.style.left 			= Obj.MWM_Bakup_style_left;
		Obj.style.width 		= Obj.MWM_Bakup_style_width +'px';
		Obj.style.height 		= Obj.MWM_Bakup_style_height +'px';
		Obj.style.marginTop 	= Obj.MWM_Bakup_style_marginTop;
		Obj.style.opacity 	 	= Obj.MWM_Bakup_style_opacticy;
		Obj.style.MozOpacity 	= Obj.MWM_Bakup_style_MozOpacity;
		Obj.style.filter 		= Obj.MWM_Bakup_style_filter;
		Obj.style.zIndex 		= Obj.MWM_Bakup_style_zIndex;
		Obj.style.letterSpacing = Obj.MWM_Bakup_style_letterSpacing;
	}
}
...
function GlissementVerticalSin ( Obj , da , db ) { 
	switch ( Obj.MWM_Sens ) {
	case 0 :	var ProgressionSin = Math.sin(( Math.PI / 2 ) * ( 1 - Obj.MWM_Progression ));	break;
	case 1 :	var ProgressionSin = Math.sin(( Math.PI / 2 ) * Obj.MWM_Progression );			break;
	}
	switch ( Number( db ) ) {
...
function GlissementVerticalCos ( Obj , da , db ) { 
	switch ( Obj.MWM_Sens ) {
	case 0 :	var ProgressionSin = Math.cos(( Math.PI / 2 ) * ( 1 - Obj.MWM_Progression ));	break;
	case 1 :	var ProgressionSin = Math.cos(( Math.PI / 2 ) * Obj.MWM_Progression );			break;
	}
	switch ( Number( db ) ) {
...
function TailleHorizontale ( Obj , da , db ) { 
	var TRef = Obj.MWM_Bakup_style_width;
	var Action = Obj.MWM_Sens + ( da * 2 );

	switch ( Action ) {
	case 0 :	var ProgressionSin = Math.sin(Math.PI * Obj.MWM_Progression / 2);			TabInfoModule[Obj.id]['DimConteneurX'] = Math.floor( TRef - ( ProgressionSin * TRef * db ) );	break;
	case 1 :	var ProgressionSin = Math.sin(Math.PI * ( 1 - Obj.MWM_Progression ) / 2);	TabInfoModule[Obj.id]['DimConteneurX'] = Math.floor( TRef - ( ProgressionSin * TRef * db ) );	break;
	case 2 :	var ProgressionSin = Math.sin(Math.PI * Obj.MWM_Progression / 2);			TabInfoModule[Obj.id]['DimConteneurX'] = Math.floor( TRef + ( ProgressionSin * TRef * db ) );	break;
	case 3 :	var ProgressionSin = Math.sin(Math.PI * ( 1 - Obj.MWM_Progression ) / 2);	TabInfoModule[Obj.id]['DimConteneurX'] = Math.floor( TRef + ( ProgressionSin * TRef * db ) );	break;
	}
	CalculeDecoModule ( TabInfoModule , Obj.id );

	chaine = 'Obj.id:' + Obj.id + '; ModuleNom:' + TabInfoModule[Obj.id]['module_nom'] +'; DimConteneurX:' + TabInfoModule[Obj.id]['DimConteneurX'];
	JSJournal[dbgPopMenu](chaine);

}

function TailleVerticale ( Obj , da , db ) {
	var TRef = Obj.MWM_Bakup_style_height;
	var Action = Obj.MWM_Sens + ( da * 2 );
	switch ( Action ) {
	case 0 :	var ProgressionSin = Math.sin(Math.PI * Obj.MWM_Progression / 2);			TabInfoModule[Obj.id]['DimConteneurY'] = Math.floor( TRef - ( ProgressionSin * TRef * db ) );	break;
	case 1 :	var ProgressionSin = Math.sin(Math.PI * ( 1 - Obj.MWM_Progression ) / 2);	TabInfoModule[Obj.id]['DimConteneurY'] = Math.floor( TRef - ( ProgressionSin * TRef * db ) );	break;
	case 2 :	var ProgressionSin = Math.sin(Math.PI * Obj.MWM_Progression / 2);			TabInfoModule[Obj.id]['DimConteneurY'] = Math.floor( TRef + ( ProgressionSin * TRef * db ) );	break;
	case 3 :	var ProgressionSin = Math.sin(Math.PI * ( 1 - Obj.MWM_Progression ) / 2);	TabInfoModule[Obj.id]['DimConteneurY'] = Math.floor( TRef + ( ProgressionSin * TRef * db ) );	break;
	}
	CalculeDecoModule ( TabInfoModule , Obj.id );
...
function DilatationLetterSpacingSin ( Obj , da ) { 
	switch ( Obj.MWM_Sens ) {
	case 0 :	var ProgressionSin = Math.sin(Math.PI * Obj.MWM_Progression / 2);			break;
	case 1 :	var ProgressionSin = Math.sin(Math.PI * ( 1 - Obj.MWM_Progression ) / 2);	break;
	}
...
function DilatationLetterSpacingCos ( Obj , da ) { 
	switch ( Obj.MWM_Sens ) {
	case 0 :	var ProgressionSin = Math.cos(Math.PI * Obj.MWM_Progression / 2);			break;
	case 1 :	var ProgressionSin = Math.cos(Math.PI * ( 1 - Obj.MWM_Progression ) / 2);	break;
	}
	Obj.style.letterSpacing = '+' + ( Math.floor( ProgressionSin * Number( da ) )) + 'px';
}
...
function DilatationLineHeightSin ( Obj , da ) { 
	...
	Obj.style.lineHeight = ( Math.floor( Obj.MWM_Bakup_style_lineHeightN + ( ProgressionSin * Number( da ) ) ) ) + 'px';	//Haaaaaaaaaaaaaaaaaaaaaaaa!!!!!!!!!!!!!!!!!
	var Objex22 = Gebi ( Obj.id + '_ex22' );
...
function DilatationLineHeightCos ( Obj , da ) { 
	...
	Obj.style.lineHeight = ( Math.floor( Obj.MWM_Bakup_style_lineHeightN + ( ProgressionSin * Number( da ) ))) + 'px';
	var Objex22 = Gebi ( Obj.id + '_ex22' );
	Objex22.style.lineHeight = Obj.style.lineHeight
}
