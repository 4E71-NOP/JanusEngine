...
if ($GALdebug == 1) {
	$GAL_nom = "Debug";
	$GAL_dir = "../websites-datas/www.rootwave.net/data/document/fra_galerie_dessin_peinture_p01/";
...
if (is_dir($GAL_dir)) {
	...
	while (false !== ($file = readdir($handle))) {
		if ($file != "." && $file != ".." && strpos($file, $website['ws_gal_fichier_tag']) === false) {
			$GAL_dir_array[] = $file;
		}
	}
	closedir($handle);
	...
	if ($GALdebug == 1) {
		outil_debug($GAL_dir_array, "\$GAL_dir_array");
	}
	...
	if ($GAL_nbr_page_reste != 0) {
		$GAL_nbr_page++;
	}											// Ajoute 1 si la division ne tombe pas juste
	...
	for ($GAL_page = 1; $GAL_page <= $GAL_nbr_page; $GAL_page++) {
		if ($_REQUEST['GAL_page_selection'] != $GAL_page) {
			...
			echo ("<span class='" . $theme_tableau . $_REQUEST['bloc'] . "_tb3 " . $theme_tableau . $_REQUEST['bloc'] . "_highlight'>(" . $GAL_page . ")</span>&nbsp;&nbsp;");
		}
	}
