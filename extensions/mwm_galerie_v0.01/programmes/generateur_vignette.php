...
if ( $_REQUEST['GAL_debug'] == 1 ) {
	error_reporting( E_ERROR | E_WARNING | E_PARSE | ~E_NOTICE );							// http://fr2.php.net/error_reporting
...
if ( $pv['compteur'] < 100 ) {
	...
	switch ($GAL_['fichier_extenssion']) {
		...
		default:
			$GAL_['thumbnail_temp'] = imagecreatetruecolor($_REQUEST['x'], $_REQUEST['y']);
			$GAL_['bg'] = imagecolorallocate($GAL_['thumbnail_temp'], 255, 255, 255);
			imagefilledrectangle ($GAL_['thumbnail_temp'], 0, 0, $_REQUEST['x'], $_REQUEST['y'], $GAL_['bg']);
			$GAL_['textcolor'] = imagecolorallocate($GAL_['thumbnail_temp'], 0, 0, 0);
			imagestring($GAL_['thumbnail_temp'], 5, ($_REQUEST['x']/2)-4 , ($_REQUEST['y']/2)-4 , "?", $GAL_['textcolor']);
			$GAL_['textcolor'] = imagecolorallocate($GAL_['thumbnail_temp'], 255, 0, 0);
			imagestring($GAL_['thumbnail_temp'], 5, ($_REQUEST['x']/2)-5 , ($_REQUEST['y']/2)-5 , "?", $GAL_['textcolor']);
			imageJPEG($GAL_['thumbnail_temp']);
			exit(0);
	}
