...
class InteractiveElements {
	...
	public function renderIconSelectFile ($infos){
		...
		$contenu_A = "
			...
			<img src='../graph/" . $ThemeDataObj->getThemeDataEntry('theme_repertoire') . "/".$ThemeDataObj->getThemeBlockEntry($infos['blockT'],'icone_repertoire') . "' width='".$X."' height='".$Y."' border='0'>
			</span>
			";
		
		$contenu_B = "
			<span class='".$Block."_lien ".$Block."_t3' Onclick=\"document.forms['".$i['formName']."'].elements['".$i['formTargetId']."'].value = '';\">
			<img src='../graph/".$ThemeDataObj->getThemeDataEntry('theme_repertoire')."/".$ThemeDataObj->getThemeBlockEntry($infos['blockT'],'icone_efface') . "' width='".$X."' height='".$Y."' border='0'>
			</span>\r
			";
	...
	<img src='../graph/" . ${$theme_tableau}['theme_repertoire'] . "/" . ${$theme_tableau}[$_REQUEST['blocT']]['icone_repertoire'] . "' width='".$X."' height='".$Y."' border='0'></span>\r
	";
	...
	CDMExec.FormCible = '".$FormNom."';
	".$JavascriptRoutine."();\">\r
	<img src='../graph/" . ${$theme_tableau}['theme_repertoire'] . "/" . ${$theme_tableau}[$_REQUEST['blocT']]['icone_efface'] . "' width='".$X."' height='".$Y."' border='0' alt=''></span>\r
	";
