...
// 			$CMobj = ConfigurationManagement::getInstance ();
			...
			$LMObj->InternalLog( array( 'level' => loglevelStatement, 'msg' => "checkUserCredential mode:".$mode));
			switch ($mode) {
				case "session" :
...
// 					}
					$LMObj->InternalLog( array( 'level' => loglevelStatement, 'msg' => "session checkUserCredentials :            user_login='".$UserObj->getUserEntry ('user_login')."'"));
					$LMObj->InternalLog( array( 'level' => loglevelStatement, 'msg' => "session checkUserCredentials :    session user_login='".$_SESSION['user_login']."'"));
					$LMObj->InternalLog( array( 'level' => loglevelStatement, 'msg' => "session checkUserCredentials :         user_password='".$UserObj->getUserEntry ('user_password')."'"));
// 					$LMObj->InternalLog( array( 'level' => loglevelStatement, 'msg' => "session checkUserCredentials : session user_password='".$_SESSION['user_password']."'");
				...
				case "form" :
					if ( $RequestDataObj->getRequestDataSubEntry('authentificationForm', 'user_login') != "anonymous") {
						...
						$LMObj->InternalLog( array( 'level' => loglevelStatement, 'msg' => "form checkUserCredentials :         user_login='".$UserObj->getUserEntry ('user_login')."'"));
						$LMObj->InternalLog( array( 'level' => loglevelStatement, 'msg' => "form checkUserCredentials :    form user_login='".$RequestDataObj->getRequestDataSubEntry('authentificationForm', 'user_login')."'"));
						$LMObj->InternalLog( array( 'level' => loglevelStatement, 'msg' => "form checkUserCredentials :      user_password='".$UserObj->getUserEntry ('user_password')."'"));
						$LMObj->InternalLog( array( 'level' => loglevelStatement, 'msg' => "form checkUserCredentials : form user_password='".hash("sha512",stripslashes($RequestDataObj->getRequestDataSubEntry('authentificationForm', 'user_password')))."'"));
			...
			if ( $this->data['error'] === TRUE ) {
				foreach ( $this->data['errorInternalLog'] as $A ) { $LMObj->InternalLog( array( 'level' => loglevelStatement, 'msg' => "Athentification Error : " . $A )); } // only for debug purpose
				$SMObj->ResetSession();
				$CurrentSetObj = CurrentSet::getInstance();
				...
				$LMObj->InternalLog( array( 'level' => loglevelStatement, 'msg' => "checkUserCredential = The user `".$this->report['user_login']."` has successfuly authenticated"));
