...
class ThemeDescriptor {
	...
	public function getThemeDescriptorDataFromDB($ThemeId, $UserObj, $WebSiteObj) {
		...
		if ( $SDDMObj->num_row_sql($dbquery) != 0 ) {
			$LMObj->InternalLog("ThemeDescriptor/getThemeDescriptorDataFromDB() : Loading data for theme descriptor id=".$ThemeId);
		}
		else {
			$dbquery = $SDDMObj->query("
			SELECT *
			...
			WHERE theme_id = 2
			;");
			$LMObj->InternalLog("ThemeDescriptor/getThemeDescriptorDataFromDB() : No rows returned for theme descriptor id=".$ThemeId.".Fallback on generic theme.");
		}
