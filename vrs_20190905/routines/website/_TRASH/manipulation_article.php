...
//		Creation de article
		case 1:
			...
			if ( $R['ERR'] != 1 ) {
				...
				$R['arti_id'] = manipulation_trouve_id_suivant ( $SqlTableListObj->getSQLTableName('article') , "arti_id" );
				...
				$requete1 = "INSERT INTO ".$SqlTableListObj->getSQLTableName('article')." VALUES ".$requete_valeurs.";";
...
//	Modification de l'article et mise a jour des infos
		...
		if ( $R['validation_etat'] == 1 ) { 
			...
			if ( $R['ERR'] != 1 ) {
				...
				$requete1 = "UPDATE ".$SqlTableListObj->getSQLTableName('article')." SET ".$requete_colonne." WHERE arti_id = '".$R['id']."';";
				manipulation_traitement_requete ( $requete1 );
...
//	Suppression de l'article
		case 3:
			...
			if ( $R['ERR'] != 1 ) {
				$requete1 = "DELETE FROM ".$SqlTableListObj->getSQLTableName('article')." 
				WHERE arti_id = '".$R['id']."'
				AND site_id ='".$_REQUEST['site_context']['site_id']."'
				;";
				manipulation_traitement_requete ( $requete1 );
				$requete1 = "DELETE FROM ".$SqlTableListObj->getSQLTableName('article_tag')." 
				WHERE arti_id = '".$R['id']."'
				;";
				manipulation_traitement_requete ( $requete1 );
...
//		Ligature avec un article
		case 4:
			...
			if ( $R['ERR'] != 1 ) {
				$requete1 = "UPDATE ".$SqlTableListObj->getSQLTableName('article')." SET 
				docu_id = '".$R['docu_id']."' 
				WHERE arti_id = '".$R['id']."'
				;";
				manipulation_traitement_requete ( $requete1 );
