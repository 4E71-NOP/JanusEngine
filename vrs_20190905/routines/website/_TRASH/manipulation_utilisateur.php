...
//	Creation
		case 1: 
			...
			if ( $R['ERR'] == 0 ) {
				...
				$R['user_id'] = manipulation_trouve_id_suivant ( $SqlTableListObj->getSQLTableName('user') , "user_id" );
				...
				$requete1 = "INSERT INTO ".$SqlTableListObj->getSQLTableName('user')." VALUES ".$requete_valeurs.";";
				...
				$R['id2'] = manipulation_trouve_id_suivant ( $SqlTableListObj->getSQLTableName('groupe_user') , "groupe_user_id" );
				$requete2 = "
				INSERT INTO ".$SqlTableListObj->getSQLTableName('groupe_user')." VALUES ( 
				'".$R['id2']."',
				'".$MU_forge_groupe."',
				'".$R['user_id']."',
				'1'
...
//	Modification
		case 2:
			...
			if ( !isset($R['confirmation_modification']) && $_REQUEST['contexte_d_execution'] == "Admin_menu" ) { 
				...
				journalisation_evenement ( 1 , $_REQUEST['sql_initiateur'] , $WebSiteObj->getWebSiteEntry('site_nom')." : ".$_REQUEST['tampon_commande'] , "ERR" , "M_UTILIS_2_0004" , $tl_[$l]['M_UTILIS_2_04'] );
			}
			...
			if ( $R['ERR'] != 1 ) {
				...
				$requete1 = "UPDATE ".$SqlTableListObj->getSQLTableName('user')." SET ".$requete_colonne." WHERE user_id = '".$R['user_id']."';";
				manipulation_traitement_requete ( $requete1 );
				...
				journalisation_evenement ( 1 , $_REQUEST['sql_initiateur'] , $WebSiteObj->getWebSiteEntry('site_nom')." : ".$_REQUEST['tampon_commande'] , "OK" , "M_UTILIS_2_0003" , $tl_[$l]['M_UTILIS_2_03'] );
				$_REQUEST['CC']['status'] = "OK";
...
*/
			...
			if ( $R['ERR'] == 0 ) {
				if ( $R['confirmation_suppression'] == 1 ) {
					$requete1 = "
					UPDATE ".$SqlTableListObj->getSQLTableName('user')." 
					SET user_status = '2' 
		...
		case 4:
			...
			if ( $R['ERR'] == 0 ) {
				...
				if ( $R['user_groupe_premier'] == 1 ) {
					$requete1 = "UPDATE ".$SqlTableListObj->getSQLTableName('groupe_user')." SET
					groupe_premier = '0' 
					WHERE user_id = '".$R['user_id']."'
					;";
					manipulation_traitement_requete ( $requete1 );
				}
				$_REQUEST['M_UTILIS_groupe_user_id']	= manipulation_trouve_id_suivant ( $SqlTableListObj->getSQLTableName('groupe_user') , "groupe_user_id" );
				$requete1 = "
				INSERT INTO ".$SqlTableListObj->getSQLTableName('groupe_user')." VALUES (
...
//	show
		case 5:
			...
			if ( $R['user_login'] == "NA" ) {
				...
				FROM ".$SqlTableListObj->getSQLTableName('user')." as usr , ".$SqlTableListObj->getSQLTableName('groupe_user')." as gu , ".$SqlTableListObj->getSQLTableName('site_groupe')." as sg 
