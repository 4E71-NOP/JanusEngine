...
// --------------------------------------------------------------------------------------------
/*Hydre-licence-fin*/
// Creation d'elements a la demande des pages si necessaire

// CreationElement = OBSOLETE : A VIRER LORS DE LA MISE A JOUR DE CETTE PAGE.
// FICHIER A SUPPRIMER

foreach ( $_REQUEST['CreationElement'] as $A ) {
	switch 	( $A['type'] ) {
	case "div":
		echo ("
		<div id='".$A['id']."' class='".$theme_tableau.$_REQUEST['bloc']."_t3 ".$theme_tableau.$_REQUEST['bloc']."_fca' 
		style=' position:absolute; display: none; visibility: hidden; z-index: 99; overflow: scroll; width: ".$A['width']."; height: ".$A['height']."px'></div>
		");
	break;
	case "uni_gestion_des_decorations":
	$pv['nbr_lignes'] = 32;
		$pv['nbr_cellules'] = 8;
		$tl_['eng']['CE1'] = "Select the image to replace the element.";
		$tl_['fra']['CE1'] = "S&eacute;lectionnez l'image qui va remplacer l'&eacute;l&eacute;ment.";
		$tl_['eng']['CE_fermer'] = "['X'] Close";
		$tl_['fra']['CE_fermer'] = "['X'] Fermer";
		echo ("			
		<div id='".$A['id']."' class ='".$theme_tableau.$_REQUEST['bloc']."_t3 ".$theme_tableau.$_REQUEST['bloc']."_fca' 
		style=' position:absolute; display: none; visibility: hidden; z-index: 99; overflow: hidden; width: ".$A['width']."px; height: ".$A['height']."px;  
		border-style:solid; border-width: 5px; border-color: #000000;'>

		<table style ='border-spacing: 1px; empty-cells: hide; vertical-align: top; width: ".($A['width'] )."px;'>\r
		<tr>\r<td class='".$theme_tableau.$_REQUEST['bloc']."_tb2 ".$theme_tableau.$_REQUEST['bloc']."_fcta' style='text-align: right;'>
		<a class='".$theme_tableau.$_REQUEST['bloc']."_lien' OnClick=\"javascript:CommuteAffichage( '".$A['id']."' )\">".$tl_[$l]['CE_fermer']."</a>\r</td>\r</tr>\r
		<tr>\r<td class='".$theme_tableau.$_REQUEST['bloc']."_t3 ".$theme_tableau.$_REQUEST['bloc']."_fctb'>".$tl_[$l]['CE1']."</td>\r</tr>\r
		</table>\r

		<div style=' position:absolute; z-index: 99; overflow: scroll; width: ".($A['width'] )."px; height: ".$A['height']."px;'>
		<table style ='border-spacing: 1px; empty-cells: hide; vertical-align: top;'>\r
		");
		$pv['n1'] = 1;
		$pv['n2'] = 1;
		for ( $pv['i'] = 1; $pv['i'] <= $pv['nbr_lignes'] ; $pv['i']++ ) {
			echo ("<tr>\r");
			for ( $pv['j'] = 1; $pv['j'] <= $pv['nbr_cellules'] ; $pv['j']++ ) {
				echo ("<td class='".$theme_tableau.$_REQUEST['bloc']."_t1 ".$theme_tableau.$_REQUEST['bloc']."_fcb' id='".$A['Cellnom'] . "G" . $pv['n1']."' style='width: 64px; height: 64px;'></td>\r");  
				$pv['n1']++;
			}
			echo ("</tr>\r");
			echo ("<tr>\r");
			for ( $pv['j'] = 1; $pv['j'] <= $pv['nbr_cellules'] ; $pv['j']++ ) {
				echo ("<td class='".$theme_tableau.$_REQUEST['bloc']."_t1 ".$theme_tableau.$_REQUEST['bloc']."_fcc'>
				<div id='".$A['Cellnom'] . "T" . $pv['n2']."' style='width: 64px; overflow: hidden;'>
				 </td>\r");
				$pv['n2']++;
			}
			echo ("</tr>\r");
	...
	case "ColoPicker":
		...
		<td><div id='MWM_CP_PR_PR' style='width: 300px; height: 64px; background-image: url(../graph/universel/palette_rapide.png);'></div></td>\r
		<td><div id='MWM_CP_PR_icone_mode' style='width: 64px; height: 64px; background-image: url(../graph/universel/icone_switch_colorpicker.png); background-color: #A0A0A0; background-repeat: no-repeat; background-position: center;'></div></td>\r
		</tr>\r
		</table>\r
		</div>\r
		");
		...
		background-image: url(../graph/universel/colorpicker_degrade_alpha.png); background-color: #FF0000; border-color: #000000; border: 1px; border-style: solid;'></div>

		<div id='MWM_CP_PC_hue_curseur' style='position: absolute; left: ".($cpd_hue['x']-6)."px; top: ".($cpd_hue['y']-2)."px; width: 46px; height: 6px; 
		background-image: url(../graph/universel/HUE_curseur_001.png);'></div>
		<div id='MWM_CP_PC_hue' style='position: absolute; left: ".$cpd_hue['x']."px; top: ".$cpd_hue['y']."px; width: 32px; height: 252px; 
		background-image: url(../graph/universel/hue.png); border-color: #000000; border: 1px; border-style: solid;'></div>
		...
		<div id='MWM_CP_PC_icone_mode' style='position: absolute; left: ".$cpd_swm['x']."px; top: ".$cpd_swm['y']."px;	width: 16px; height: 16px; background-image: url(../graph/universel/icone_switch_colorpicker.png); background-repeat: no-repeat; background-position: center;'>
		</div>\r
		");
		break;
	}
