...
function CalculeDecoModule ( Table , ModuleNom ) {
	var TIM = Table[ModuleNom];
	var T = 0; 
	l.Log[dbgCalcDeco](TIM.module_nom + '/' + TIM.deco_type);
	...
	case 40:
		...
		for ( var Div in ListeDivsGebi ) {
			...
			T.DivObj = elm.Gebi( ModuleNom + '_' + ListeDivsGebi[Div] );
		...
		for ( var Div in ListeDivsGebi ) {
			var Obj = elm.Gebi( ModuleNom + '_' + ListeDivsGebi[Div] );
	...
	case 50:
		...
		for ( var Div in ListeDivsGebi ) {
			var Obj = Gebi( ModuleNom + '_' + ListeDivsGebi[Div] );
			Obj.style.left = TIM[ListeDivsGebi[Div]].PosX + 'px';
			Obj.style.top = TIM[ListeDivsGebi[Div]].PosY + 'px';
			Obj.style.width = TIM[ListeDivsGebi[Div]].DimX + 'px';
			Obj.style.height = TIM[ListeDivsGebi[Div]].DimY + 'px';
	...
	if  ( dbgCalcDeco == 1 ) { console.table(TIM); }
}
