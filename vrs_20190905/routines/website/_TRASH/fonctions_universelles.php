...
$LMObj->logCheckpoint("Fonctions_universelles");
$MapperObj->RemoveThisLevel($localisation );
$MapperObj->setSqlApplicant("Fonctions_universelles");



$_REQUEST['localisation'] .= $localisation;
// statistique_checkpoint ("fonctions_universelles");
$LMObj->logCheckpoint("fonctions_universelles", $T, $MapperObj);
...
function genere_table_langue ( $tab ) {
	...
	switch ( $_REQUEST['contexte_d_execution'] ) {
	case "Installation":
		$dbquery = requete_sql($_REQUEST['sql_initiateur'],"SELECT * FROM ".$SqlTableListObj->getSQLTableName('langues').";");
	...
	case "Rendu":
	default:
		$dbquery = requete_sql($_REQUEST['sql_initiateur'],"SELECT * FROM ".$SqlTableListObj->getSQLTableName('site_langue')." WHERE site_id = '".$_REQUEST['sw']."';");
...
function journalisation_evenement ( $Type , $initiateur , $action , $signal , $msgid , $text ) {
	...
	case "Rendu":
	default:
		global $WebSiteObj, $SQL_tab, $db , $db_;
		...
		switch ( $_REQUEST['debug_option']['journalisation_cible'] ) {
		case "systeme" :
			$A = "MWM_Engine_log: ".$WebSiteObj->getWebSiteEntry('sw_id')."|".time ()."|".$initiateur."|".$action."|".$signal."|".$msgid."|".$text;
			error_log ( html_entity_decode ( $A ) ,0 );
		break;
		case "echo" :
			echo ( "MWM_Engine_log: ".$WebSiteObj->getWebSiteEntry('sw_id')."|".time ()."|".$initiateur."|".$action."|".$signal."|".$msgid."|".$text ."<br>" );
		break;
		case "interne" :
		default:
...
function Extension_Appel ( $nom  ) {
	...
	FROM ".$SqlTableListObj->getSQLTableName('extension_fichiers')." exf 
	...
	FROM ".$SqlTableListObj->getSQLTableName('extension_config')." exc 
	WHERE exc.extension_id = '".$tab['extension_id']."' 
	AND exc.site_id = '".$WebSiteObj->getWebSiteEntry('sw_id')."' 
	;";
	$dbquery = requete_sql($_REQUEST['sql_initiateur'], $pv['requete'] );
	while ($dbp = fetch_array_sql($dbquery)) { 
...
class UniversalFunctions {
	...
	public static function getInstance() {
		if (self::$Instance == null) {
			self::$Instance = new UniversalFunctions();
		}
		return self::$Instance;
	}
	
	
	public function conversion_expression ( $expr ) {
		global $tab_conv_expr;
		$a = strtolower($expr);
		$expr2 = $tab_conv_expr[$_REQUEST['conv_expr_section']][$a];
		if ( $_REQUEST['debug_conversion_expression'] == 1 ) { echo ("Section ".$_REQUEST['conv_expr_section']."; in: ".$expr." / ".$a." / out: ".$expr2." <br>"); }
