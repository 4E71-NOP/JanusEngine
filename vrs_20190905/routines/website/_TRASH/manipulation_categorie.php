...
function MC_reconstruit_branche () {
	...
	if ( $MC_position_err['signal'] == 1 ) {
		foreach ( $liste_categorie_ as $B ) { 
			...
			UPDATE ".$SqlTableListObj->getSQLTableName('categorie')." 
...
function MC_cate_devient_dernier() {
	...
	$R = &$_REQUEST['M_CATEGO'];
	requete_sql( $_REQUEST['sql_initiateur'], "
	UPDATE ".$SqlTableListObj->getSQLTableName('categorie')." SET 
	cate_position 	= '100', 
	derniere_modif	= '".time()."' 
	WHERE cate_id	= '".$R['id']."' 
	;");
			...
			if ( $R['ERR'] != 1 ) {
				if ( !isset($R['position']) ) {
					$function_parametres = array (
						...
						"SQL_tab_categorie" => $SqlTableListObj->getSQLTableName('categorie'),
						"parent" 			=> $R['parent'],
				...
				$R['id'] = manipulation_trouve_id_suivant ( $SqlTableListObj->getSQLTableName('categorie') , "cate_id" );
				...
				$requete1 = "INSERT INTO ".$SqlTableListObj->getSQLTableName('categorie')." ".$requete_colonne." VALUES ".$requete_valeurs.";";
...
//	modification de categorie
		case 2:
			...
			if ( $R['ERR'] != 1 && $R['confirmation_modification'] != 0 ) {
				...
				$requete1 = "UPDATE ".$SqlTableListObj->getSQLTableName('categorie')." SET ".$requete_colonne." WHERE cate_id = '".$R['id']."';";
				manipulation_traitement_requete ( $requete1 );
