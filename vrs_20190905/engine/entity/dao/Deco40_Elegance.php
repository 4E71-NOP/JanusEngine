...
class Deco40_Elegance {
	...
	public function getDeco40_EleganceDataFromDB($id) {
		...
		if ( $SDDMObj->num_row_sql($dbquery) != 0 ) {
			$LMObj->InternalLog( array( 'level' => LOGLEVEL_STATEMENT, 'msg' => __METHOD__ . " : Loading data for deco_40_elegance id=".$id));
			while ( $dbp = $SDDMObj->fetch_array_sql ( $dbquery ) ) {
				$this->Deco40_Elegance[$dbp['deco_variable_name']] = $dbp['deco_value'];
			}
		}
		else {
			$LMObj->InternalLog( array( 'level' => LOGLEVEL_STATEMENT, 'msg' => __METHOD__ . " : No rows returned for deco_40_elegance id=".$id));
		}
