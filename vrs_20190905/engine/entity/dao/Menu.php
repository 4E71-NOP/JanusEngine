...
class Menu extends Entity
{
	...
	{
		$res = true;
		if ($this->Menu['lang_id'] == 0) {
			$res = false;
		}
		if ($this->Menu['deadline_id'] == 0) {
			$res = false;
		}
		if ($this->Menu['menu_state'] < 0 && $this->Menu['menu_state'] > 2) {
			$res = false;
		}
		if ($this->Menu['group_id'] == 0) {
			$res = false;
		}
		if ($this->Menu['menu_last_modif'] == 0) {
			$res = false;
		}
		if ($this->Menu['menu_parent'] != 0 || $this->Menu['menu_parent'] != null) {
			if ($this->entityExistsInDb('menu', $this->Menu['menu_parent']) == false) {
				$res = false;
			}
		}
		if ($this->entityExistsInDb('website', $this->Menu['ws_id']) == false) {
			$res = false;
		}
		if ($this->entityExistsInDb('group', $this->Menu['fk_group_id']) == false) {
			$res = false;
		}
		if ($this->entityExistsInDb('deadline', $this->Menu['fk_deadline_id']) == false) {
			$res = false;
		}
		if ($this->entityExistsInDb('permission', $this->Menu['fk_perm_id']) == false) {
			$res = false;
		}
