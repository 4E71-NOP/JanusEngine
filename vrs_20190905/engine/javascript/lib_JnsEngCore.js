...
// 'cfg'(as JnsEngGlobalConfig type).
...
class ElementHandling {
	...
	SetAdminSwitchLocation(Div, Position, dimX, dimY) {
		...
		if (Obj) {
			...
			l.Log[cfg.CoreDbg]("Div" + Div + " is missing (" + typeof Obj + ").");
		}
	}
	...
	LocateMouse(e) {
		...
		if (now > this.mouseData.Frequency) {
			for (var i in this.mouseFunctionList) {
				// l.Log[cfg.MouseDbg]("LocateMouse : " + this.mouseFunctionList[i].obj+"; method:"+this.mouseFunctionList[i].method);
				var obj = this.mouseFunctionList[i].obj;
				obj.MouseEvent();
			}
			this.mouseData.lastExec = MTime;
		}
		l.Log[cfg.MouseDbg]("Type:" + de.cliEnv.browser.support + "; Mouse Location:PosX=" + this.mouseData.PosX + "PosY=" + this.mouseData.PosY);
	}
...
var cfg = new JnsEngGlobalConfig();
var l = new Log();
