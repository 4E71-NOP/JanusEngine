...
class  RenderTables
{
	...
	public function render($infos, $T)
	{
		...
		for ($CurT = 1; $CurT <= $tab_infos['NbrOfTabs']; $CurT++) {
			...
			switch ($ADC['tabs'][$CurT]['TableCaptionPos']) {
				case 1:
					$ADC['tabs'][$CurT]['legendClasses'] .= $Block . _CLASS_TBL_LGND_TOP_;
					break; // top
				case 2:
					$ADC['tabs'][$CurT]['legendClasses'] .= $Block . _CLASS_TBL_LGND_LEFT_;
					break; // left
				case 3:
					$ADC['tabs'][$CurT]['legendClasses'] .= $Block . _CLASS_TBL_LGND_RIGHT_;
					break; // right
				case 4:
					$ADC['tabs'][$CurT]['legendClasses'] .= $Block . _CLASS_TBL_LGND_BOTTOM_;
					break; // bottom
				case 5:
					$ADC['tabs'][$CurT]['legendClasses'] .= $Block . _CLASS_TBL_LGND_LEFT_ . " " . $Block . _CLASS_TBL_LGND_RIGHT_;
					break; // left&right
				case 6:
					$ADC['tabs'][$CurT]['legendClasses'] .= $Block . _CLASS_TBL_LGND_TOP_ . " " . $Block . _CLASS_TBL_LGND_BOTTOM_;
					break; // top&bottom
			}
		}
		// --------------------------------------------------------------------------------------------
		...
		for ($CurT = 1; $CurT <= $tab_infos['NbrOfTabs']; $CurT++) {
			...
			if ($ADC['tabs'][$CurT]['NbrOfLines'] != 0) {
				...
				if (isset($ADC['tabs'][$CurT]['HighLightType'])) {
					$tab_infos['HighLightType'] = $ADC['tabs'][$CurT]['HighLightType'];
				}
				...
				if (isset($AD[$CurT]['caption']['cont'])) {
					if (isset($AD[$CurT]['caption']['class'])) {
						$captionClass = "class='" . $AD[$CurT]['caption']['class'] . "' ";
					}
					if (isset($AD[$CurT]['caption']['syle'])) {
						$CaptionStyle = "style='" . $AD[$CurT]['caption']['style'] . "' ";
					}
				...
				for ($CurL = 1; $CurL <= $ADC['tabs'][$CurT]['NbrOfLines']; $CurL++) {
					if ($CurL == $ADC['tabs'][$CurT]['theadD']) {
						$Content .= "<thead>\r";
					}
					if ($CurL == $ADC['tabs'][$CurT]['tbodyD']) {
						$Content .= "<tbody style='display:block; width:100%; height:" . ($tab_infos['Height'] - 64) . "px; overflow-y:scroll;'>\r";
					}		//display:block;
					if ($CurL == $ADC['tabs'][$CurT]['tfootD']) {
						$Content .= "<tfoot>\r";
					}
					...
					for ($CurC = 1; $CurC <= $ADC['tabs'][$CurT]['NbrOfCells']; $CurC++) {
						if ($AD[$CurT][$CurL][$CurC]['disabled'] == 0) {
							...
							if (isset($AD[$CurT][$CurL][$CurC]['cc'])) {
								$strDeco .= $AD[$CurT][$CurL][$CurC]['cc'];
							}
							if (isset($AD[$CurT][$CurL][$CurC]['class'])) {
								$strDeco .= $AD[$CurT][$CurL][$CurC]['class'] . " ";
							}
							if ($strDeco == " class='") {
								$strDeco = "";
							} else {
								$strDeco = substr($strDeco, 0, -1) . "'";
							}

							if (isset($AD[$CurT][$CurL][$CurC]['style'])) {
								$strDeco .= " style='" . $AD[$CurT][$CurL][$CurC]['style'] . "'";
							}

							$tdTmp = "<td" . $strDeco;
							$spanScore = 0;
							if ($AD[$CurT][$CurL][$CurC]['colspan'] != 0) {
								$spanScore += 1;
							}
							if ($AD[$CurT][$CurL][$CurC]['rowspan'] != 0) {
								$spanScore += 2;
							}
							switch ($spanScore) {
								case 1:
					...
					$Content .= "</tr>\r";
					if ($CurL == $ADC['tabs'][$CurT]['theadF']) {
						$Content .= "</thead>\r";
					}
					if ($CurL == $ADC['tabs'][$CurT]['tbodyF']) {
						$Content .= "</tbody>\r";
					}
					if ($CurL == $ADC['tabs'][$CurT]['tfootF']) {
						$Content .= "</tfoot>\r";
					}
					$TRidx ^= 1;
				}
	...
	public function getDefaultDocumentConfig(&$infos, $lines = 0, $NbrOfTabs = 1, $TabBehavior = 1, $HighLightType = 1, $TabTxt = "tabTxt")
	{
		...
		$table_tr_height = $CurrentSetObj->ThemeDataObj->getThemeBlockEntry($infos['blockT'], "table_tr_height");
		if (empty($table_tr_height)) { $table_tr_height = _TABLE_TR_HEIGHT_; } // Fail safe
		...
		$tab = array(
			...
			"Height"			=> $lines * $table_tr_height,		// default
