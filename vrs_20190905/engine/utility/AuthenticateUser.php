	...
	public function checkUserCredential(User $UserObj, $mode)
	{
		...
		if (strlen($UserObj->getUserEntry('error_login_not_found') ?? '') == 0) {
			...
			switch ($mode) {
				...
				case "form":
					if ($bts->RequestDataObj->getRequestDataSubEntry('authentificationForm', 'user_login') != "anonymous") {
						...
						// Failsafe for disconnection process
						if (strlen($bts->RequestDataObj->getRequestDataSubEntry('authentificationForm', 'user_password') ?? "") == 0) {
							$bts->RequestDataObj->setRequestDataSubEntry('authentificationForm', 'user_password', 'no password'); // Subject to fail
						}
			...
			if ($this->data['error'] === TRUE) {
				foreach ($this->data['errorInternalLog'] as $A) {
					$bts->LMObj->msgLog(array('level' => LOGLEVEL_STATEMENT, 'msg' => __METHOD__ . " : Error. " . $A));
