...
class SessionManagement
{
	...
	public function InitializeSession()
	{
		...
		$bts->LMObj->msgLog(array('level' => LOGLEVEL_BREAKPOINT, 'msg' => __METHOD__ . " : InitializeSession has been called"));
		$currentWs = $CurrentSetObj->getDataEntry('ws'); // get the Webite
		if (strlen($currentWs ?? '') == 0) {
			$bts->LMObj->msgLog(array('level' => LOGLEVEL_WARNING, 'msg' => __METHOD__ . " : CurrentSet WebSite empty : '" . $currentWs . "'"));
			$currentWs = DEFAULT_SITE_ID;
		}
		$bts->LMObj->msgLog(array('level' => LOGLEVEL_STATEMENT, 'msg' => __METHOD__ . " : Current WebSite is : '" . $currentWs . "'"));

		$this->session[$currentWs] = array(
			"ws"						=> $currentWs,
			"SessionMaxAge"				=> $bts->CMObj->getConfigurationEntry('SessionMaxAge'),
			"user_login"				=> "anonymous",
			"err"						=> false,
			"last_REMOTE_ADDR"			=> $_SERVER['REMOTE_ADDR'],
			"last_REMOTE_PORT"			=> $_SERVER['REMOTE_PORT'],
			"last_HTTP_USER_AGENT"		=> $_SERVER['HTTP_USER_AGENT'],
			"last_REQUEST_TIME"			=> $_SERVER['REQUEST_TIME'],
			"currentRoute"				=> array("target" => "home", "page" => 1),
			"previousRoute"				=> array("target" => "home", "page" => 1),
		);
	}
		...
		if (strlen($currentWs ?? '') > 0) {
			...
			if (isset($_SESSION[$currentWs]['last_REMOTE_ADDR'])) {
				if ($_SESSION[$currentWs]['last_REMOTE_ADDR'] != $_SERVER['REMOTE_ADDR']) {
					...
					$this->report['errMsg'] = "Not the same remote IP address";
					$bts->LMObj->msgLog(array('level' => LOGLEVEL_STATEMENT, 'msg' => __METHOD__ . " : Not the same remote IP address"));
				}
			} else {
				$_SESSION[$currentWs]['last_REMOTE_ADDR'] = $_SERVER['REMOTE_ADDR'];
			}
			...
			if (isset($_SESSION[$currentWs]['last_HTTP_USER_AGENT'])) {
				...
				$_SESSION[$currentWs]['last_HTTP_USER_AGENT'] = $_SERVER['HTTP_USER_AGENT'];
			}
			...
			if (isset($_SESSION[$currentWs]['last_REQUEST_TIME'])) {
				...
				$_SESSION[$currentWs]['last_REQUEST_TIME'] = $_SERVER['REQUEST_TIME'];
			}
			...
			$bts->LMObj->msgLog(array('level' => LOGLEVEL_STATEMENT, 'msg' => __METHOD__ . " : \$CurrentSetObj->getDataEntry('ws') is empty... problem."));
			$bts->LMObj->msgLog(array('level' => LOGLEVEL_WARNING, 'msg' => __METHOD__ . " : No session check has been performed."));
		}
	}
	...
	public function getSessionEntry($data)
	{
		return $this->session[$data];
	}
	public function getSessionSubEntry($lvl1, $lvl2)
	{
		return $this->session[$lvl1][$lvl2];
	}
	public function getSession3rdLvlEntry($lvl1, $lvl2, $lvl3)
	{
		return $this->session[$lvl1][$lvl2][$lvl3];
	}
	...
	public function setSessionSubEntry($lvl1, $lvl2, $data)
	{
		$this->session[$lvl1][$lvl2] = $data;
	}
	public function setSession3rdLvlEntry($lvl1, $lvl2, $lvl3, $data)
	{
		$this->session[$lvl1][$lvl2][$lvl3] = $data;
	}
