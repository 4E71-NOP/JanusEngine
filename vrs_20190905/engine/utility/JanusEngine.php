...
class JanusEngine
{
	...
	public function render()
	{
		...
		// if (empty($_SESSION[$wsSession])) {
			...
			if (!file_exists($configFile)) {
				// JnsEng config should have the necessary privileges to log to the DB and retrieve the default_website definition.
				$bts->LMObj->msgLog(array('level' => LOGLEVEL_STATEMENT, 'msg' => __METHOD__ . " : Definition is empty. Falling back to 'JnsEngCore'."));
		...
		if ($this->initializeSDDM() == true) {
			...
			$this->GeneratedScript->insertString('JavaScript-OnLoad', "\telm.Gebi('JnsEngBody').style.visibility = 'visible';");
	...
	private function initializeJavascript()
	{
		...
		$this->GeneratedScript->insertString('JavaScript-File', 'current/engine/javascript/lib_JnsEngCore.js');
