...
class CommandConsole
{
	...
	private function commandInitialization(&$CCL)
	{
		...
		switch (strtolower($CCL['init']['cmd'])) {
			case "update":
				foreach ($CCL["params"] as &$C) {
					$C = "";
				}	// We clear any default value as it's an update command. So we can only select what's necessary to update later. We keep the array so we can check if a given value (by the command) doesn't fit
				break;
			case "add":
			default:
				break;
		...
		foreach ($CCL['incoming'] as $A => $B) {
			$CCL['params'][$A] = $bts->SDDMObj->escapeString($B);
		}
		$bts->LMObj->msgLog(array('level' => LOGLEVEL_BREAKPOINT, 'msg' => __METHOD__ . " : End"));
	}
						...
						case 2:
							...
							if ($q != -1) {
								...
								if ($bts->SDDMObj->num_row_sql($dbquery) == 0) {
									...
									while ($dbp = $bts->SDDMObj->fetch_array_sql($dbquery)) {
										$CCL['params'][$A['v']] = $dbp[$A['c']];
										$bts->LMObj->msgLog(array('level' => LOGLEVEL_BREAKPOINT, 'msg' => __METHOD__ . " ['params']['" . $A['v'] . "']='" . $CCL['params'][$A['v']] . "'."));
									}
								}
							}
							break;
	...
	private function prepareSQLStatement(&$CCL)
	{
		...
		if (is_array($ptr['convert'])) {
			foreach ($ptr['convert'] as $A) {
				$CCL['params'][$A['v']] = $bts->StringFormatObj->conversionExpression($CCL['params'][$A['v']], $A['s']);
			}
		}
		//----------------------------------------
		...
		if (is_array($ptr['langConvert'])) {
			foreach ($ptr['langConvert'] as $A) {
				$CCL['params'][$A['t']] = $bts->CMObj->getLanguageListSubEntry($A['v'], 'lang_id');
			}
		}
		//----------------------------------------
		...
		// Stuffing object to privide SDDM function if necessary.
		// The callable function has nothing in the scope and is alone in the universe.
		$CCL['SddmToolsObj'] = $bts->SddmToolsObj;
	...
	public static function getReportEntry($a)
	{
		return self::$report[$a];
	}
