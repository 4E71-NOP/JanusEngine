...
self::$CheckTable['add']['website']['0']['f']	= function ($a) {return array("SELECT CONCAT('0x', HEX(ws_id)) AS ws_id, ws_name FROM " . $a['sqlTables']['website'] . " WHERE ws_name = '" . $a['params']['name'] . "';");};
self::$CheckTable['add']['website']['0']['m']	= "CLI_Website_C001";
self::$CheckTable['add']['website']['0']['s']	= "name";
self::$CheckTable['add']['website']['1']['d']	= _RETURN_DATA_AND_ERROR_;
self::$CheckTable['add']['website']['1']['f']	= function ($a) {return array("SELECT CONCAT('0x', HEX(lang_id)) AS lang_id FROM " . $a['sqlTables']['language'] . " WHERE lang_639_3 = '" . $a['params']['lang'] . "'");};
...
self::$CheckTable['update']['website']['0']['d']	= _RETURN_DATA_AND_ERROR_;
...
self::$CheckTable['update']['website']['1']['f']	= function ($a) {return array("SELECT CONCAT('0x', HEX(lang_id)) AS lang_id FROM " . $a['sqlTables']['language'] . " WHERE lang_639_3 = '" . $a['params']['lang'] . "';");};
...
self::$CheckTable['update']['website']['2']['d']	= _RETURN_DATA_AND_ERROR_;
self::$CheckTable['update']['website']['2']['f']	= function ($a) {return array("SELECT CONCAT('0x', HEX(theme_id)) AS theme_id, sd.theme_name FROM " . $a['sqlTables']['theme_descriptor'] . " sd, " . $a['sqlTables']['theme_website'] . " ss WHERE sd.theme_name = '" . $a['params']['theme'] . "' AND sd.theme_id = ss.fk_theme_id AND ss.fk_ws_id = " . $a['Context']['ws_id'] . ";");};
...
// Site Context
self::$CheckTable['website']['context']['0']['d']	= _RETURN_DATA_AND_ERROR_;
self::$CheckTable['website']['context']['0']['f']	= function ($a) {return array("SELECT CONCAT('0x', HEX(ws_id)) AS ws_id FROM " . $a['sqlTables']["website"] . " WHERE ws_name = '" . $a['params']['name'] . "' ;");};
