...
self::$CheckTable['assign']['user_permission']['0']['d']	= _RETURN_DATA_AND_ERROR_;
self::$CheckTable['assign']['user_permission']['0']['f']	= function ($a) {return array("SELECT CONCAT('0x', HEX(prm.perm_id)) AS perm_id FROM " . $a['sqlTables']['permission'] . " prm WHERE prm.perm_name = '" . $a['params']['name'] . "' ;");};
...
self::$CheckTable['assign']['user_permission']['1']['f']	= function ($a) {return array("SELECT CONCAT('0x', HEX(usr.user_id)) AS user_id, usr.user_login AS user_login FROM " . $a['sqlTables']['user'] . " usr, " . $a['sqlTables']['group_user'] . " gu, " . $a['sqlTables']['group_website'] . " sg WHERE usr.user_login = '" . $a['params']['to_user'] . "' AND usr.user_id = gu.fk_user_id AND gu.fk_group_id = sg.fk_group_id AND gu.group_user_initial_group = '1' AND sg.fk_ws_id = " . $a['Context']['ws_id'] . ";");};
