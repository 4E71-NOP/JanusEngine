...
$buttonUp = "'><img src='" . $CurrentSetObj->ServerInfosObj->getServerInfosEntry('baseUrl') . "media/theme/" . $ThemeDataObj->getDefinitionValue('directory') . "/" . $ThemeDataObj->getThemeBlockEntry($infos['blockT'], 'icon_top') . "' width='24' height='24'>";
$buttonDown = "'><img src='" . $CurrentSetObj->ServerInfosObj->getServerInfosEntry('baseUrl') . "media/theme/" . $ThemeDataObj->getDefinitionValue('directory') . "/" . $ThemeDataObj->getThemeBlockEntry($infos['blockT'], 'icon_bottom') . "' width='24' height='24'>";
...
foreach ($tabListMenu as $A) {
	...
	// $bts->LMObj->msgLog(array('level' => LOGLEVEL_BREAKPOINT, 'msg' => __METHOD__ . "Tab=" . $bts->StringFormatObj->print_r_debug($Tab)));
	// $bts->LMObj->msgLog(array('level' => LOGLEVEL_BREAKPOINT, 'msg' => __METHOD__ . "l=" . $bts->StringFormatObj->print_r_debug($l)));
	...
	$T['Content'][$Tab][$l]['1']['cont'] = "<a href='"
		...
		. $A['menu_name']
		. "</a>\r";
	// Menus which do not have a parent (level 0); the table column is null so it has to be handled
	$mp = ( is_null($A['menu_parent']) || empty($A['menu_parent']) ) ? "-" : $tabListMenu[$A['menu_parent']]['menu_name']; 
	$T['Content'][$Tab][$l]['2']['cont'] = $A['menu_title'];
	$T['Content'][$Tab][$l]['3']['cont'] = $mp;
	...
	$T['Content'][$Tab][$l]['6']['cont'] = $stateTab[$A['menu_state']];
	$l++;
