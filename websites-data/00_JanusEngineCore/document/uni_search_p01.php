...
$bts->RequestDataObj->setRequestDataEntry(
	'searchForm',
	array(
		"searchType"	=>	"A",
		"search"		=>	"utilisat",
...
if (strlen($bts->RequestDataObj->getRequestDataSubEntry('searchForm', 'search') ?? '') > 3) {
	...
	if ($bts->SDDMObj->num_row_sql($dbquery) > 0) {
			...
			foreach ($A as $B) {
				...
				switch ($bts->RequestDataObj->getRequestDataSubEntry('searchForm', 'searchType')) {
					case "A":
						...
						if ($pv['position_expr'] <= ($pv['taille_extrait'] / 2)) {
							$pv['extrait_debut'] = 0;
						} else {
							$pv['extrait_debut'] = $pv['position_expr'] - ($pv['taille_extrait'] / 2);
						}
		...
		$T['ContentCfg']['tabs']['1']['NbrOfLines'] = $ligne - 1;
