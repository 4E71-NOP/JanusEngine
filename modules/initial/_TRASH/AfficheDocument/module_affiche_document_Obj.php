...
// 		$LMObj->setInternalLogTarget("both");
		...
		$i18n = array();
		include ("../modules/initial/AfficheDocument/i18n/".$l.".php");
		
		if (!class_exists("DocumentData")) { include ("../modules/initial/AfficheDocument/DocumentData.php"); }
...
// 		$CurrentSetObj->setDataSubEntry('block_HTML', 'post_hidden_sw',			"<input type='hidden'	name='sw'			value='".$WebSiteObj->getWebSiteEntry('sw_id')."'>\r" );
...
// 					if (!class_exists("RenderTables")) { include ("routines/website/utility/RenderTables.php"); }
			...
			switch ( $DocumentDataObj->getDocumentDataEntry('arti_menu_style') ) {
				...
				case "2":
					...
					$ContentMenu = "<div id='document_menu' class='" . $Block."_div_std' style='float:".$DocumentDataObj->getDocumentDataEntry('arti_menu_float_position')."; ".$tab_float_['taille_x']." ".$tab_float_['taille_y']." padding: 10px;'>\r" . $ContentMenu . "</div>\r";
					break;
			}
		}
		// --------------------------------------------------------------------------------------------
...
//		$LMObj->logDebug($analyse_document, "\$analyse_document");
		...
		if ( $SDDMObj->num_row_sql($dbquery) == 0 ) {
			$SDDMObj->query("
			INSERT INTO ".$DocumentDataObj->getDocumentDataEntry('stat_document')." VALUES (
		...
		elseif ( $SDDMObj->num_row_sql($dbquery) == 1 ) {
			while ($dbp = $SDDMObj->fetch_array_sql($dbquery)) { $pv['arti_count'] = $dbp['arti_count']; }
			$pv['arti_count']++;
			$SDDMObj->query("
			UPDATE ".$DocumentDataObj->getDocumentDataEntry('stat_document')." SET
			arti_count = '".$DocumentDataObj->getDocumentDataEntry('arti_count')."'
			WHERE arti_id = '".$DocumentDataObj->getDocumentDataEntry('arti_id')."'
			AND arti_page = '".$DocumentDataObj->getDocumentDataEntry('arti_page')."'
			;");
		}
...
// 		$LMObj->logDebug( $CurrentSetObj->getData() , "\$CurrentSetObj->getData()");
	...
	private function documentConvertion (&$inputContent, $infos) {
		...
		$s[$ptr] = "[POPIMG_L]";	$r[$ptr] = "<a target='_NEW' href='../websites-datas/".$WebSiteObj->getWebSiteEntry('sw_repertoire')."/data/documents/".$directory."/";			$ptr++;
		$s[$ptr] = "[/POPIMG_L]";	$r[$ptr] = "'>";			$ptr++;
		
		$s[$ptr] = "[POPIMG_I]";	$r[$ptr] = "<img src='../websites-datas/".$WebSiteObj->getWebSiteEntry('sw_repertoire')."/data/documents/".$directory."/";			$ptr++;
		...
		$s[$ptr] = "[IMGSRC]";		$r[$ptr] = "<img src='../websites-datas/".$WebSiteObj->getWebSiteEntry('sw_repertoire')."/data/documents/".$directory."/";			$ptr++;
