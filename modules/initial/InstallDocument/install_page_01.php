...
class InstallPage01 {
	...
	private function serverInfos($infos, $t) {
		...
		if ( intval(str_replace( "M", "", $ServerInfosObj->getServerInfosEntry('memoryLimit') )) < 128 ) { $T[$t][$l]['2']['cont'] .= " (<span class='".$Block."_warning'>".$bts->I18nTransObj->getI18nTransEntry('test_nok')."</span>)"; }
	...
	private function siteSelection($infos, $t) {
		...
		foreach ( $FormDirectoryNameList as $B ) { $str .= "\"".$B."\", \r"; }
		$str = substr($str, 0,-3) . "\r];\r";
